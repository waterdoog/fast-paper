...
\midrule
\texttt{realistic}   & $2\times2$ $\times$ $E$          & 72  & Main decomposition: $0.248 / 0.066 / \approx\!0$ \\
...
\ifcompact\input{sections/fig-ladder}\fi

\subsection{How the headline cells are computed}
\label{app:aggregation}

Every number in the $2\times2$ (Figure~\ref{fig:twobytwo}) is the mean over \emph{delivered}
cold-beat episodes of \texttt{realistic} and \texttt{axis2x2} pooled---the two runs share a
design, and pooling doubles the cells to 35--36 episodes each. Intervals are percentile
bootstrap over episodes, resampled jointly across cells so that a contrast between two
means is resampled as a contrast.

An earlier draft reported this table from \texttt{axis2x2} alone with lost beats scored as
$F_1 = 0$. One lost cold beat in cell $A$ moved that cell by $0.046$ and both diagonals with
it, which is the failure mode \S\ref{sec:quantifying} warns about appearing in our own
headline table. The values here are the corrected ones.
...
The scenario predictions of \S\ref{sec:combination} require three task families rather
than one. Each keeps forced-value scoring and hidden-profile allocation; what changes is where
the difficulty sits.
...
\subsection{The retrieval model behind the scaling question}
\label{app:retrieval}

\input{sections/fig-rho}

Let $k$ be the number of search results a requester acts on, $m$ the number of cards
matching a query, $h$ the number of those that actually hold the needed fact, and $\rho$
the correlation between a card's rank and holding it. With no ranking signal the needed
holder's expected rank is $(m+1)/2$; with perfect ranking it is $(h+1)/2$. Interpolating
between the two by $\rho$ and truncating at $k$ gives the survival probability plotted in
Figure~\ref{fig:rho}. We take $m(N) \approx 0.094N$ from the directory generator, which
draws 35\% of filler tags from the scenario vocabulary, and hold $k=20$.

The model is deliberately crude---it assumes a single query and ignores reformulation---and
its purpose is not to predict a number but to establish an order of magnitude for the
requirement on $\rho$. That requirement is the load-bearing claim: at $10^6$ cards the
ranking has to be right to within one part in a thousand, which is not a threshold a
better similarity function reaches.
