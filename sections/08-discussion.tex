...
The cold-phase crossing between regimes follows from the fact allocation and is not a
discovery; only its location and cost are measured. Forced-value deliverables cannot
capture whether an artifact is well argued, the price of removing the judge from every
primary metric. The kernel is a reference implementation, not a production deployment:
the interventions are drawn from deployed source but the runs are not deployment
traces. And one model family carries the sweeps, so relational sensitivity may be
model-specific.
